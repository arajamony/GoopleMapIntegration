...
class MainActivity : AppCompatActivity() {
    ...
    private fun getDirectionURL(origin: LatLng, dest: LatLng): HashMap<String, String> {
        val ai = packageManager.getApplicationInfo(packageName, PackageManager.GET_META_DATA)
        val bundle = ai.metaData
        val myApiKey = bundle.getString("com.google.android.geo.Direction_API_KEY")

        val Filter = HashMap<String, String>()
        Filter["startPoint"] = "${origin.latitude},${origin.longitude}"
        Filter["endPoint"] = "${dest.latitude},${dest.longitude}"
        Filter["key"] = myApiKey
        return Filter
    }

    private fun drawDirection(queryValue: HashMap<String, String>) {

        val googleMapService: GoogleMapService = ServiceBuilder.buildService(GoogleMapService::class.java)
        val requestCall: Call<ResponseBody> = googleMapService.getDirections(queryValue)
    ...
    private fun DrawPolyLine(result: List<List<LatLng>>) {
        ...
        if (!result.isNullOrEmpty()) {
            googleMap.addMarker(
                MarkerOptions().position(result.first().first()).icon(
                    BitmapDescriptorFactory.fromResource(
                        R.drawable.home_address_24
                    )
                )
            )
            googleMap.addMarker(
                MarkerOptions().position(result.last().last()).icon(
                    BitmapDescriptorFactory.fromResource(
                        R.drawable.destination_24
                    )
                )
            )
